...
from ..utils.api_call import process_text
import zipfile # <--- 【新增】在文件顶部导入 zipfile 模块
import glob # <--- 【新增】导入 glob 模块
import subprocess
# 在 services.py 文件顶部需要导入 request，调用cosyvoice
...
# 上述修改需要前端在发送请求时，使用 FormData 对象来同时提交普通表单数据和文件。我们稍后会调整前端JS。
# def start_paper_processing(pdf_path, pdf_filename, video_duration='medium', voice_type='female'):
# 【修改】函数签名，增加 output_format 参数
def start_paper_processing(pdf_path, pdf_filename, video_duration='medium', voice_type='female', output_format='video'):
    """启动论文处理流程"""
    import time
    ...
    job_info = {
        ...
        'final_output_path': None, # <--- 【新增】用于存储最终输出路径（视频或压缩包）
        'error': None,
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            if chosen_format == 'markdown':
                ...
                # --- 这部分是您原来就有的视频处理逻辑，我们将其原封不动地搬到这里 ---
                ...
                # ... (这里省略了原来很长的 cover 文件复制和生成逻辑, 请您将那部分代码完整地粘贴到此处)
                # 您原来的代码从 "code_dir = f'{output_dir}/final_results/Code'" 开始，到 "render_preview_video(process_id, base_name)" 结束
                
                # 为了完整性，我将这部分逻辑从您给出的代码中复制过来
                ...
                # 我想在这里插入自动插入用户上传的背景图（将在temp下），如果用户没有选择则不用传，
...
# 修改函数签名，增加 unique_base_name 参数
def start_folder_processing(folder_path, folder_name, unique_base_name, video_duration='medium', voice_type='female', output_format='video'):
# def start_folder_processing(folder_path, folder_name, video_duration='medium', voice_type='female', output_format='video'):
    """
    启动整个文件夹的处理流程。
    这个函数接收的是一个文件夹路径。
    """
    """
    启动整个文件夹的处理流程。
    【已更新】此版本包含与单篇处理完全一致的自定义音色处理逻辑。
    """
    import time
    from werkzeug.utils import secure_filename

    # 模仿单篇处理，添加完全相同的入口日志，确认函数被调用且参数正确
    ...
    # base_name = secure_filename(folder_name) # 使用安全的文件名作为基础
    # 【核心修改】直接使用从路由层传递过来的唯一基础名
    base_name = unique_base_name 
    # 【新增】完全复用单篇处理中的自定义音色文件和文本处理逻辑
    custom_voice_path = None
    ...
    job_info = {
        ...
        'custom_voice_path': custom_voice_path, # <-- 保留
        'custom_voice_text': custom_voice_text, # <-- 保留
        'status': 'starting',
        'progress': 0,
        'start_time': datetime.now().isoformat(),
        'last_update': datetime.now().isoformat(),
...
def run_folder_processing(process_id, folder_path, base_name):
    """
    运行基于文件夹的初始处理流程。
    它的逻辑与 run_initial_processing 非常相似，但传递给脚本的是文件夹路径。
    """
    # 内部辅助函数，保持不变
    def update_job_status(status=None, progress=None, step=None, log_msg=None, error=None, output_dir=None, stage=None):
        with processing_lock:
    ...
    try:
        ...
        if process.stdout:
            for line in process.stdout:
                line = line.strip()
                if line:
                    # 这个日志会告诉你 all_pipeline_video.py 内部在干什么
...
# 新函数添加到 services.py 的末尾，processing_jobs 声明之后，但在 Web 编辑器功能之前比较合适。
# 在 services.py 文件中，建议放在论文处理流程相关函数区域
