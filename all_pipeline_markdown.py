...
def run_single_file_workflow(input_pdf_path, output_dir=None):
    ...
    command = ["python3", "master_pipeline.py", md_path, image_output_dir, "--run-until-step", "1"] # <--- 【核心修改】
    if output_dir:
...
def run_multi_file_workflow(input_dir_path, output_dir=None):
    ...
    command = ["python3", "multi_paper_master_pipeline​.py", mineru_clean_output_dir, "--run-until-step", "1"] # <--- 【核心修改】
    if output_dir:
        command.extend(["--output-base-dir", output_dir])
        
    run_command_live_output(command, cwd="Paper2Video")
    return "多篇论文批量(仅章节切分)" # 返回处理模式用于最终输出
