...
def step3_batch_coding(split_pages_dir, generated_code_dir, chapter_type):
    ...
    print_step(3, "批量代码生成", f"使用batch_coder.py为{chapter_type}章节页面生成PPT代码")
    
    print("[LIST] 批量代码生成说明:")
    print("   • 使用GPT-4.5为每个页面生成PPT的Python代码")
    print("   • 自动处理图片引用和尺寸适配")
    ...
    success = run_command(command, "为每个页面生成对应的PPT Python代码", real_time_output=True)
    if not success:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Chapter_Agent 完整Pipeline - 从学术论文到教学PPT的自动化处理",
